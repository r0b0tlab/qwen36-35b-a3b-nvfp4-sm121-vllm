...
if [[ "${1:-}" == "audit" || "${1:-}" == "--audit-only" ]]; then
  exec /usr/bin/python3 /opt/r0b0tlab/audit_runtime_v025.py
fi
if [[ "${1:-}" == "bash" || "${1:-}" == "sh" ]]; then
  exec "$@"
fi
...
/usr/bin/python3 /opt/r0b0tlab/audit_runtime_v025.py
...
args=(
  ...
  --linear-backend "${LINEAR_BACKEND:-flashinfer_cutlass}"
...
if [[ -n "${QUANTIZATION:-modelopt_mixed}" ]]; then
  args+=(--quantization "${QUANTIZATION:-modelopt_mixed}")
fi
