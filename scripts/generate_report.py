...
def main():
    ...
    base_ctx=maxctx.get("base_ar") or {}
    mtp_ctx=maxctx.get("mtp_k2") or {}
    raw=json.dumps({"gsm8k":gsm,"concurrency":conc,"llama_benchy":llama,"telemetry":tele,"energy":energy,"runtime":manifest,"max_context":maxctx},indent=2,sort_keys=True)
...
<section class="hero"><div class="brand">r0b0tlab performance dossier</div><h1>Qwen3.6 35B-A3B<br>NVFP4 on GB10</h1><p class="subtitle">A correctness-first, SM121-native serving study: official NVIDIA ModelOpt mixed FP8/NVFP4 weights, native FlashInfer/CUTLASS kernels, FP8 KV, measured MTP K=2 behavior, and the full architectural context window.</p><div class="badges"><span class="badge">NVIDIA GB10 · SM121</span><span class="badge">vLLM 0.25.0 · CUDA 13.0</span><span class="badge">FlashInfer/CUTLASS</span><span class="badge">MTP K=2</span><span class="badge">65,536-token throughput profile</span><span class="badge">262,144-token c1 context profile</span></div></section>
<section class="grid4"><div class="kpi"><div class="l">GSM8K 0-shot</div><div class="v">{fmt((flex or 0)*100,1,'%')}</div></div><div class="kpi"><div class="l">Peak measured output</div><div class="v">{fmt(best.get('output_throughput_tok_s'),0)}</div><small>tok/s at c{best.get('concurrency','—')}</small></div><div class="kpi"><div class="l">MTP acceptance</div><div class="v">{fmt((acceptance or 0)*100,1,'%')}</div></div><div class="kpi"><div class="l">Runtime</div><div class="v">0.25.0</div><small>vLLM · CUDA 13.0</small></div></section>
...
<h2>Full architectural context</h2><div class="callout"><span class="status">262,144 tokens validated</span><p>A separate c1 context profile used 6 GiB of explicit FP8 KV rather than the c32 throughput profile's aggregate KV allocation. Live chat-template tokenization passed 65K, 131K, 196K, and the 262K ceiling for both base AR and production MTP K=2.</p></div><div class="two"><div class="panel"><h3>Base AR</h3><div class="kpi"><div class="l">Measured KV capacity</div><div class="v">{fmt(base_ctx.get('measured_kv_capacity_tokens'),0)}</div></div><p>Five-position near-window retrieval, ordered dual-code retrieval, and 512 forced output tokens passed. Minimum available host memory: {fmt(base_ctx.get('minimum_mem_available_gib'),2)} GiB.</p></div><div class="panel"><h3>MTP K=2</h3><div class="kpi"><div class="l">Measured KV capacity</div><div class="v">{fmt(mtp_ctx.get('measured_kv_capacity_tokens'),0)}</div></div><p>The same gates passed with {fmt((mtp_ctx.get('acceptance_rate') or 0)*100,2,'%')} long-context acceptance and {fmt(mtp_ctx.get('minimum_mem_available_gib'),2)} GiB minimum available host memory.</p></div></div><div class="panel"><table><thead><tr><th>Contract</th><th>Result</th></tr></thead><tbody><tr><td>Near-window prompt</td><td>261,368–261,373 tokens</td></tr><tr><td>Retrieval positions</td><td>begin · quarter · middle · three-quarter · end</td></tr><tr><td>Ordered dual-code retrieval</td><td>Passed</td></tr><tr><td>Prompt + forced generation</td><td>261,371 + 512 = 261,883 tokens</td></tr><tr><td>Remaining window margin</td><td>261 tokens</td></tr><tr><td>Added swap</td><td>0 MiB</td></tr></tbody></table><p class="muted">Claim boundary: functional near-window retrieval and bounded generation, not a general reasoning-quality benchmark over arbitrary 262K prompts.</p></div>
<h2>Methodology & reproducibility</h2><div class="panel"><p><strong>Concurrency:</strong> selected c1, c8, and c32 points; random 2,048-token input / exact 512-token output; three repetitions, first dropped and final two averaged. <strong>Quality:</strong> full GSM8K 0-shot flexible extraction with zero retries. <strong>Runtime:</strong> one GB10, text-only model path, a 65,536-token c32 throughput profile plus a separate 262,144-token c1 context profile, and no Marlin or emulation fallback. Equivalent completed measurements are hash-reused; the clean image is checked with semantic, long-generation, and c1/c32 canaries rather than a duplicate full evaluation.</p><p>Run ID: <code>{html.escape(run.name)}</code> · Model revision: <code>{html.escape(str(manifest.get('model_revision','—')))}</code> · Image: <code>{html.escape(str(manifest.get('image_id','—')))}</code></p></div>
