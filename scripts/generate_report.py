...
<section class="hero"><div class="brand">r0b0tlab performance dossier</div><h1>Qwen3.6 35B-A3B<br>NVFP4 on GB10</h1><p class="subtitle">A correctness-first, SM121-native serving study: official NVIDIA ModelOpt mixed FP8/NVFP4 weights, native FlashInfer/CUTLASS MoE, FP8 KV baseline, and measured MTP behavior.</p><div class="badges"><span class="badge">NVIDIA GB10 · SM121</span><span class="badge">vLLM 0.24 · CUDA 13</span><span class="badge">FlashInfer/CUTLASS</span><span class="badge">MTP ×2</span><span class="badge">65,536-token profile</span></div></section>
...
<h2>What actually runs</h2><div class="two"><div class="panel stack"><div class="step"><div class="num">01</div><div><h3>Mixed checkpoint</h3><p>ModelOpt mixed FP8 dense groups plus NVFP4 expert weights.</p></div></div><div class="step"><div class="num">02</div><div><h3>Native target kernels</h3><p>FlashInferCutlass NVFP4 GEMM and explicit FlashInfer/CUTLASS target MoE on <code>sm_121a</code>.</p></div></div><div class="step"><div class="num">03</div><div><h3>Correct MTP split</h3><p>The selected NVIDIA ModelOpt profile and measured MTP behavior are recorded here; the MTP draft path is retained only if its counters and semantics pass.</p></div></div></div><div class="panel"><h3>Resolved runtime</h3><table><tr><td>KV cache</td><td>FP8</td></tr><tr><td>Attention</td><td>FlashInfer</td></tr><tr><td>Target MoE</td><td>FLASHINFER_native target</td></tr><tr><td>Draft MoE</td><td>TRITON</td></tr><tr><td>CUDA graphs</td><td>PIECEWISE (effective)</td></tr><tr><td>GPU KV capacity</td><td>{fmt(manifest.get('kv_cache_tokens'),0)} tokens</td></tr></table></div></div>
